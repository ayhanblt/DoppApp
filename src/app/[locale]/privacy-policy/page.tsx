    ...
    <div className="min-h-screen bg-zinc-50 pt-24 pb-12 px-4 sm:px-6 lg:px-8 relative">
      ...
      <div className="max-w-3xl mx-auto bg-white rounded-3xl shadow-sm border border-zinc-100 overflow-hidden">
        <div className="bg-[var(--accent)] px-8 py-10 text-white flex flex-col items-center text-center">
          ...
          <h1 className="text-3xl font-black">{t.privacyPolicyTitle}</h1>
        </div>
        
        <div className="p-8 sm:p-12">
          <div className="space-y-6 text-zinc-600 leading-relaxed text-base">
            {t.privacyPolicyContent.map((paragraph, index) => {
              const isNumbered = /^\d+\./.test(paragraph);
              return (
                <div key={index} className={isNumbered ? "ml-4 mt-8" : ""}>
                  {isNumbered ? (
                    <>
                      <h3 className="font-bold text-zinc-900 text-lg mb-2">{paragraph.split(':')[0]}</h3>
